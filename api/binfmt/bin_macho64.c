...
/* =========================================================================
   This file implement functions for parsing Mach-O (64 bits) binaries
   ======================================================================= */


/* Check if the binary is an Mach-O 64 file */
static int r_binfmt_macho64_is(r_binfmt_s *bin) {
  ...
  if(hdr->h_magic == R_BINFMT_MACHO64_MAGIC ||
     hdr->h_magic == R_BINFMT_MACHO64_CIGAM)
    return 1;
...
static r_binfmt_endian_e r_binfmt_macho64_getendian(r_binfmt_s *bin) {
  r_binfmt_macho64_header_s *hdr;
  ...
  if(hdr->h_magic == R_BINFMT_MACHO64_CIGAM)
...
static void r_binfmt_macho64_load_segments(r_binfmt_s *bin) {
  r_binfmt_macho64_header_s *hdr;
...
r_binfmt_err_e r_binfmt_macho64_load(r_binfmt_s *bin) {
  ...
  if(!r_binfmt_macho64_is(bin))
    return R_BINFMT_ERR_UNRECOGNIZED;

  bin->type = R_BINFMT_TYPE_MACHO64;
  bin->endian = r_binfmt_macho64_getendian(bin);
  bin->arch = r_binfmt_macho64_getarch(bin);
  ...
  if(!r_binfmt_macho64_check(bin))
    return R_BINFMT_ERR_MALFORMEDFILE;

  bin->entry = 0;
  r_binfmt_macho64_load_segments(bin);
